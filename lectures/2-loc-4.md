...
$$
Y^\star = \arg\min_{Y \subseteq N}
\left\{ \sum_{i \in Y} k_i
+ \sum_{i \in Y} \sum_{j \in M_i} c_{ij}
\;:\; \bigcup_{i \in Y} M_i = M \right\}
\tag{1}
$$
...
- $Y$ = sites at which NFs are located, $Y \subseteq N$
- $Y^\star$ = minimum cost set of sites where NFs are located
- $|Y^\star|$ = number of NFs located.
...
$$
Y^\star = \arg\min_{Y \subseteq N}
\left\{ \sum_{i \in Y} \sum_{j \in M_i} c_{ij}
\;:\; \bigcup_{i \in Y} M_i = M, \; |Y| = p \right\}
\tag{2}
$$
...
$$
m_{\min} = \lfloor r_a\,t_e + 1 \rfloor ,
\tag{3}
$$
...
$$
\begin{aligned}
TPC_\text{act} &= \max_{f < f_\text{max}}
\left\{ TPC_\text{min},\; TPC_0 \left( \frac{f}{f_0} \right)^{\beta} \right\},\\[4pt]
\beta &= \begin{cases}
0.62, & \text{Hand tool mfg.}\\
0.48, & \text{Construction}\\
0.41, & \text{Chemical processing}\\
0.23, & \text{Medical centers}
\end{cases}
\end{aligned}
\tag{4}
$$
...
$$
\boxed{\;TPC_\text{est} = \textcolor{#d21f26}{k} + c_p f\;}
\tag{5}
$$

$$
APC_\text{act} = \frac{TPC_\text{act}}{f}
= \frac{TPC_0}{f_0^{\,\beta}}\, f^{\,\beta-1}
\tag{6}
$$

$$
APC_\text{est} = \frac{k}{f} + c_p
\tag{7}
$$

where

- $f$ = production rate, in units per year
- $\beta$ = scale exponent, below one under economies of scale2
- $k$ = fixed cost
- $c_p$ = constant unit production cost
- $f_\text{min}/f_\text{max}$ = min/max feasible scale
- $f_\text{MES}$ = minimum efficient scale
- $TPC_0/f_0$ = base cost/rate.
...
- $x_i, y_i$ = predictor and observed response of observation $i$
- $\widehat{y}_i$ = response the line predicts at $x_i$
- $\alpha_1, \alpha_2$ = intercept and slope of the line
- $\alpha_1^\star, \alpha_2^\star$ = coefficients minimizing the loss.
...
$$
\begin{aligned}
\text{L1 loss: } \sum \bigl| \widehat{y} - y \bigr|
&\implies \text{Minimum Absolute Deviation (MAD)}\\
&\implies \text{Median}\\[6pt]
\text{L2 loss: } \sum \bigl( \widehat{y} - y \bigr)^2
&\implies \text{Minimum Square Error (MSE)}\\
&\implies \text{Mean}
\end{aligned}
\tag{10}
$$
